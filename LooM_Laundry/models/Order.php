...
class Order{
    ...
    public function markReceived(){
        ...
        $query = "SELECT * FROM Orders WHERE Serviced_Vendor=:id AND Order_Status ='Pending Collection' OR Order_Status ='In Transit'";
        $statement = $this->conn->prepare($query);
    ...
    public function zipcodes(){
        $query = "SELECT zipcode FROM Vendor";
        $statement = $this->conn->prepare($query);
    ...
    public function getVendorUsingZipCode($zipcode){
        $query = "SELECT V_ID FROM Vendor WHERE V_ZipCode = :zipcode";
        $statement = $this->conn->prepare($query);
    ...
    public function getVendorZipcode(){
        $query = "SELECT V_ZipCode FROM Vendor";
        $statement = $this->conn->prepare($query);
        $statement->execute();
        $result=$statement->fetchAll(\PDO::FETCH_ASSOC);
        $zipcodes = array();
        foreach($result as $result){
            array_push($zipcodes,$result["V_ZipCode"]);
        }
        $vendorZipcode = $this->getClosest($_SESSION["zip-code"],$zipcodes);
        $result = $this->getVendorUsingZipCode($vendorZipcode);
        $vendorID = 0;
        // foreach($result as $result){
        //     $vendorID = $result["V_ID"];
        // }
        // return $vendorID;
        return $result["V_ID"];
    ...
    public function getClosest($value, $vendorIds) {
        $closest = null;
        foreach ($vendorIds as $item) {
